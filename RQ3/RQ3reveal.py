...
train_data, test_data = train_test_split(
    reveal_data, test_size=0.2, random_state=seed, stratify=reveal_data['target']
)
...
# 确保保存目录存在
save_directory = 'model/reveal/rq3'
...
# 设置是否启用对比学习
use_contrastive_learning = False  # 可以设置为 False 禁用对比学习
...
for epoch in range(epochs):
    ...
    for step, batch in enumerate(progress_bar):
        ...
        with autocast():
            ...
            # 总损失
            total_loss_value = classification_loss + 0.1 * contrastive_loss_value
    ...
    # 验证集评估
    acc, prec, rec, f1, avg_loss, _ = evaluate_model(model, valid_loader)
    logging.info(f"Validation - Accuracy: {acc:.4f}, Precision: {prec:.4f}, Recall: {rec:.4f}, F1: {f1:.4f}")
